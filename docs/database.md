...
- **Category**: exclusão é física (hard delete), permitida apenas quando não há produtos vinculados. Não utiliza `deletedAt`. Ver `decisions.md`.
- **Sale / SaleItem / Rental / RentalItem**: não possuem operação de exclusão ou cancelamento nesta etapa (registros imutáveis após criados). Não utilizam `deletedAt`. Ver `decisions.md`.
